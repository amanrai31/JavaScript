...
1. JS is only language that a browser understands. (We have other languages that transpiled to JS e.g. TS, Dart, WebAssembly). HTML connects the JS to browser.
...
**NOTE :** If you write script tag in head, you woun't be able to access document elements too early(as html is still parshing) ``` console.dir(document.body); // will give nothing ``` => That's why we put script at the end.
